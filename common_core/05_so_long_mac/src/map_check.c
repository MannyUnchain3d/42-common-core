...
/*   Updated: 2022/12/17 18:23:47 by Manny            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_content_wall(t_map *map, int row, int col)
{
...
void	map_symbols_checker(t_map *map)
{
	...
	while (row < map->height)
	{
		...
		while (col < map->width)
		{
			...
			check_content_wall(map, row, col);
			col++;
		}
		row++;
	}
	if (checker.start == 0 || checker.exits == 0)
		display_error_exit(map, MAP_START_EXIT_ERR);
	if (checker.collectibles == 0)
		display_error_exit(map, MAP_COLLECTIBLES_ERR);
	if (checker.start > 1 || checker.exits > 1)
		display_error_exit(map, MAP_DUP_CHAR_ERR);

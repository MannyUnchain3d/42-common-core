...
/*   Updated: 2022/12/16 23:49:26 by Manny            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	skip_ending_newlines(t_map *map, int fd)
{
	char	*line;
	...
	while (line != NULL)
	{
		if (ft_strlen(line, 1) > 0)
		{
			free(line);
			close(fd);
			display_error_exit(map, MAP_SIZE_ERR);
		}
		line = get_next_line(fd);
	}
}
...
void	map_size_init(t_map *map, char *ber)
{
	...
	skip_ending_newlines(map, file.fd);
	close(file.fd);
...
void	map_init(t_map *map, char *ber)
{
	...
	file.line = skip_beginning_newlines(file.fd);
	if (file.line == NULL)
		display_error_exit(map, OTHER_ERR);
